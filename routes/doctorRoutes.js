...
router.get('/routes/current_user', (req, res) => {
  res.send(req.session);
  res.send(req.user);
});


router.post('/routes/doctorRoute', async (req, res) => {
  ...
  if (name || email || date || subscription || chat || audioCall || videoCall || isAdmin || password || repeatPassword) {
    ...
    if (password === repeatPassword) { 
      ...
      const newDoctor = new Doctor({ name, email, date, subscription, chat, audioCall, videoCall, isAdmin, newPassword });
      await newPatient.save();

      res.send(_.pick(newDoctor, ['_id', 'name', 'time', 'email', 'subscription', 'chat', 'audioCall', 'videoCall', 'isAdmin']));
    } else {
