  ...
  router.post('/', async (req, res) => {
    ...
    if (name || email || date || subscription || chat || audioCall || videoCall || password || repeatPassword) {
      ...
      if (password === repeatPassword) { 
        ...
        const newPatient = new Patient({ name, email, date, subscription, chat, audioCall, videoCall, newPassword });
        await newPatient.save();

        res.send(_.pick(newPatient, ['_id', 'name', 'time', 'email', 'subscription', 'chat', 'audioCall', 'videoCall',]));
      } else {
